...
export class FormBuilderComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.storesAttributeForm.valueChanges
  ...
  async ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.selectedAddress) {
      if (this.selectedAddress && this.storesAttributeForm) {
        this.storesAttributeForm.patchValue({
          storeCustomAttributes: [this.AddressInfo[this.selectedAddress]],
        });

        const param = {
          id: this.AddressInfo[this.selectedAddress].country_id.id,
  ...
  updateFieldList() {
    ...
    this.existingAttributesData.sort(
      ...
      .then(async (response) => {
        if (response.result.success) {
          ...
          if (
            ...
            this.module == "certificate_of_origin" ||
            this.module == "draft_bl" ||
  ...
  handleClick(formdata, index, formArray) {
    if (
      formdata.form_control_name == "country_id" ||
      formdata.form_control_name == "state_province_id"
    ) {
      this.handleAddCountry(formdata, index, formArray);
    }
    if (formdata.form_control_name == "payment_id") {
      this.handelDisplayDetials(formdata, index, formArray);
    }
  }
  handelDisplayDetials(formdata, index, formArray) {
    let dialogRef = this.dialog.open(PaymentDetailsDialog, {
      panelClass: "alert-dialog",
      width: "400px",
      autoFocus: false,
      data: {
        id: this.storesAttributeForm.controls.storeCustomAttributes[
          "controls"
        ][0].get("payment_id").value.id,
      },
    });
  }
  isSpanDisabled(formControlName: string): boolean {
    const controls =
      this.storesAttributeForm.controls.storeCustomAttributes["controls"][0];
    if (formControlName === "payment_id") {
      return !controls.get("payment_id")?.value?.id; // Disable if payment_id is null
    }
    if (formControlName === "state_province_id") {
      return !controls.get("country_id")?.value; // Disable if country_id is null
    }
    return false; // country_id is always enabled
  }

  public newCountry = "";
  handleAddCountry(formdata, index, formArray) {
    let country_id = "";
    let naming = "";
    let type = "";
    if (formdata.form_control_name === "country_id") {
      country_id = "";
      naming = "Country";
      type = "add_country";
    } else {
      country_id =
        this.storesAttributeForm.controls.storeCustomAttributes[
          "controls"
        ][0].get("country_id").value.id;
      naming = "State";
      type = "add_state";
    }
    console.log(country_id);
    let dialogRef = this.dialog.open(AddCountryStateComponent, {
      panelClass: "alert-dialog",
      width: "400px",
      data: {
        naming,
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result && result.success) {
        ...
        this.existingAttributesData.map(async (obj: any, index: number) => {
          if (obj.form_control_name == "country_id" && type == "add_country") {
            obj.options = this.dropdOptions?.[obj.form_control_name] || [];
            ...
            obj.form_control_name == "state_province_id" &&
            type == "add_state"
          ) {
            obj.options.unshift(result.response);
            this.storesAttributeForm.controls.storeCustomAttributes[
  ...
  async dropDownSelectionChange(event, formdata, index, formArray) {
    ...
    if (
      formdata.form_control_name === "type_of_payment" &&
      this.storesAttributeForm?.controls?.storeCustomAttributes[
        "controls"
      ][0]?.get("payment_id")
    ) {
    ...
    if (this.module === "commercial_invoice_add_payment") {
      if (formdata.form_control_name == "payment_id") {
        this.service
          .getPaymentBalanceById({ id: event.value.id })
          .then((response) => {
            if (response.result.success) {
              const data = response.result.data;
              this.existingAttributesData.map((obj: any) => {
                if (obj.label_dependent_field_id) {
                  const indx = obj.label_dependent_field_id.indexOf(
                    formdata.id
                  );
                  if (
                    indx > -1 &&
                    obj.label_dependent_field_id[indx] == formdata.id
                  ) {
                    obj.custom_label = data.balance_amount
                      ? "(" + data.balance_amount + ")"
                      : "";
                  }
                }
  ...
  applyCustomHtml(attr: any): string {
    let content = attr.label || attr.first_name || attr.name;

    if (attr.description_value) {
      return `<div><span class="custom-css">${content}</span>&nbsp;&nbsp;&nbsp;<span class="dropdown-new-label">(${
        attr.description_key ? attr.description_key : ""
      } - ${
        attr.description_value ? attr.description_value : ""
      })</span></div>`;
    } else {
      return `<div><span class="custom-css">${content}</span></div>`;
    }
  }
}
