...
export class IgstComponent implements OnInit {
    ...
    this.getListIGST(true)
    this.getIGSTGlobalFilters()
  ...
  getIGSTGlobalFilters():void{
    ...
    .getIGSTGlobalFilters()
	 ...
	 igstDetails(stepper: MatStepper, data: any):any{
		// this.router.navigate(['/accounts/igst', data.id]);
  ...
  public igstData:any
  getListIGST(clearList?: any):void{
    if (clearList) {
			this.fetchingData = true;
		}
    this.AccountsService
    .getListIGST(this.getPagams())
        .then(response => {
          // console.log(response)
          this.fetchingData = false;
          this.filtersLoader = false;
...
console.log(product,event,value)
	...
	history(data:any){
		...
		let dialogRef = this.dialog.open(AmountHistoryComponent, {
			panelClass: 'alert-dialog',
			width: '500px',
			data: {
				...data
