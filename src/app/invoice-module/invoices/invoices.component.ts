...
export class InvoicesComponent implements OnInit {
	...
	constructor(
		...
		// public EpiCurrencyPipe: EpiCurrencyPipe
	) { }
	...
	orderDetail() {
	  ...
	  getInvoiceFilters() {	
		this.statusSpinner = true;	
			...
			let param = {}	
			this.OrdersService	
				.getInvoiceFilterList(param)	
				.then(response => {	
					// console.log(response)	
					if(response.result.success) {	
						this.invoices.statusFilter = response.result.data.invoiceStatus;	
	...
	getOrdersList(clearList?: any, flag?): void {
		...
		this.OrdersService
			.getInvoiceList(this.pagedata)
			.then(response => {
				if (response.result.success) {
					this.fetchingData = false;
					let data = response.result.data.Invioce;
					this.listCount = data ? data.length : 0;
					this.totalCount = response.result.data ? response.result.data['count'] : 0;
					if(data && data.length) {
						data.map(res => {
