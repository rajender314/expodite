		...
		this.OrdersService
			...
			.then(response => {
				if (response.result.success) {
					...
					// this.orders.selectedClients.map(function (value) {
					// 	value.selected = false;
					// });
		...
		this.OrdersService
			...
			.then(response => {
				...
				if(response.result.data.totalordersDt.length) {
					 selectedOrderDetails = response.result.data.totalordersDt[0].list[0];
					this.orders.selectedOrder = selectedOrderDetails.orders;
					this.selectedOrderStatus = selectedOrderDetails.orders.status;
					this.orders.billingAddr = selectedOrderDetails.billingAddr;
					this.orders.shippingAddr = selectedOrderDetails.shippingAddr;
					this.po_date2 = new Date(this.orders.selectedOrder.po_date ? this.orders.selectedOrder.po_date : "");
					if (selectedOrderDetails.notifyingAddr) {
		...
		this.OrdersService
			...
			.then(response => {
				...
				if (response.result.success) {
					...
					this.searching = false;
					// if (!this.orders.status.length) {

					// 	this.orders.status = response.result.data.ordersTypesDt;
					// 	// this.orders.shippingAddress = [{ id: 64, name: this.orderFormCompanyDetails.FilterByFirstShipperAddress }, { id: 63, name: this.orderFormCompanyDetails.FilterBySecondShipperAddress }];
					// 	this.orders.status.map(function (value) {
					// 		value.selected = false;
					// 	});
					// 	this.orders.shippingAddress.map(function (value) {
					...
					// console.log(this.orders.data, this.fetchingData)
