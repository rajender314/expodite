...
export class CreateShipmentComponent implements OnInit {
  ...
  onTabChange(index: number) {
    ...
    if (index == 0) {
      ...
      const indx = _.findIndex(this.currencyList, { id: currency });
      if (indx > -1) {
      ...
      if (
        ...
        setTimeout(() => {
          if (!this.formData.valid) {
            ...
            scrollTag = document.querySelector(".address-error");
          }
          if (scrollTag) {
            scrollTag.scrollIntoView({ behavior: "smooth" });
          }
        ...
        setTimeout(() => {
          if (!this.pfiProductsPayload.valid) {
            ...
            // productItems.controls.forEach((formGroup, index) => {
            //   if (formGroup.invalid) {
            //     scrollTag = document.querySelector(`.product-error .prod-list-${index}`);
  ...
  async saveShipment(tabGroup: MatTabGroup, tabIndex) {
    // console.log(this.tabIndex, tabIndex);
    // console.log(tabGroup, tabIndex)
    //   console.log(this.pfiProductsPayload);
    //   console.log(this.pfiSubTotalPayload);
    // console.log(this.formData, this.pfiAddressPayload);
    // if (this.tabIndex == 0) {
    //   this.formData.markAllAsTouched();
    // } else {
    //   this.pfiSubTotalPayload.markAllAsTouched();
    ...
    // this.getTabIndex(tabGroup, tabIndex);
    ...
    if (
      ...
      param = {
        ...
        create_shipment: this.pfiClientPayload.value.storeCustomAttributes[0],
        pfi_address_form: this.pfiAddressPayload,
        add_product_in_shipment: this.pfiProductsPayload.value.productItem,
        shipment_payment_terms: this.paymentForm.value.storeCustomAttributes[0],
        shipment_other_specifications:
      ...
      if (this.totalClients.length) {
        const indx = _.findIndex(this.totalClients, {
          label: param.create_shipment.organization_id,
        });
        if (indx > -1) {
          param.create_shipment.organization_id = this.totalClients[indx].id;
        }
      }
      if (tabIndex != 0 && tabIndex != 1 && tabIndex != 2) {
  ...
  public productsApi(data) {
    ...
    const params = {
      order_ids: this.data?.ordersId || "",
      related_to_id: this.data.ordersId[0],
      form_id: this.estimate_form_id,
      meta_data: {
        // add_product: this.pfiProductsPayload.value.productItem,
        ...data,
      },
      id: data.title == "Edit" ? this.data.estimate_id : "",
