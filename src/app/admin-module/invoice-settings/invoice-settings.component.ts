...
export class InvoiceSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.adminService.rolePermissions.edit_invoice_settings == 1) {
      this.seriesForm.enable();
    } else if (this.adminService.rolePermissions.edit_invoice_settings == 2) {
  ...
  getQutationSettings() {
    this.adminService
      .getNumberSeriesSettings({ type: "invoice" })
      .then((response) => {
        if (response.result.success) {
