...
@Component({
  selector: "app-quotation-settings",
  templateUrl: "./quotation-settings.component.html",
  styleUrls: ["./quotation-settings.component.scss"],
})
export class QuotationSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.adminService.rolePermissions.edit_quotation_settings == 1) {
      this.seriesForm.enable();
    } else if (this.adminService.rolePermissions.edit_quotation_settings == 2) {
  ...
  getQutationSettings() {
    this.adminService
      .getNumberSeriesSettings({ type: "quotation" })
      .then((response) => {
        if (response.result.success) {
