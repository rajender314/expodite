...
export class OrderSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.adminService.rolePermissions.edit_order_settings == 1) {
      this.seriesForm.enable();
    } else if (this.adminService.rolePermissions.edit_order_settings == 2) {
  ...
  getQutationSettings() {
    this.adminService
      .getNumberSeriesSettings({ type: "orders" })
      .then((response) => {
        if (response.result.success) {
