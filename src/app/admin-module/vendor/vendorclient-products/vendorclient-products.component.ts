...
declare var App: any;
@Component({
  selector: 'app-vendorclient-products',
  templateUrl: './vendorclient-products.component.html',
  styleUrls: ['./vendorclient-products.component.scss'],
  providers: [SnakbarService],
})

export class VendorclientProductsComponent implements OnInit, OnChanges {
	...
	public uploader: FileUploader = new FileUploader({
		allowedMimeType: ['image/png', 'image/jpeg', 'image/jpg', 'application/pdf'],
	...
	constructor(
		...
		this.uploader
			.onWhenAddingFileFailed = (item: any, response: any, options: any) => {
				let toast: object;
				...
				if (item.size >= options.maxFileSize) {
					...
					toast = { msg: "You have selected an invalid file type. Only jpg, jpeg, png and pdf files are allowed.", status: "error" };
					this.snackbar.showSnackBar(toast);
				}
	...
	getUploadTla() {
		...
		this.organizationsService.getTLA({ orgs_id: this.Organization.id })
			.then(response => {
	...
	deleteTla() {
		this.organizationsService.deleteTLA({ id: this.Organization.id })
			.then(response => {

			});
	}
	getProductList() {
		this.organizationsService.getMyProductList({ org_id: this.Organization.id })
			.then(response => {
				const productList = [];
	...
	fileSelectedvalue(event): void {
		if (this.uploadData.length) {
			this.uploader.setOptions({ url: App.base_url + 'uploadTLA?orgs_id=' + this.Organization.id + '&id=' + this.uploadId });
		} else {
			this.uploader.setOptions({ url: App.base_url + 'uploadTLA?orgs_id=' + this.Organization.id + '&id=' + '' });
		}
	...
	submitValue() {
		...
		if (this.selectedChecbox1) {
			...
			this.organizationsService.addProducts(products)
				.then(response => {
					if (response.result.success) {
			...
			this.organizationsService.addProducts(productsList)
				.then(response => {
					if (response.result.success) {
						toast = { msg: " Products saved successfully.", status: "success" };
						this.snackbar.showSnackBar(toast);
