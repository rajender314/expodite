...
declare var App: any;
@Component({
  selector: 'app-vendor-certifications',
  templateUrl: './vendor-certifications.component.html',
  styleUrls: ['./vendor-certifications.component.scss']
})

export class VendorCertificationsComponent implements OnInit {
	@Input() Organization;
	...
	constructor(private organizationsService: OrganizationsService, private snackbar: SnakbarService, private _lightbox: Lightbox, public dialog: MatDialog,) {
		...
		this.uploader
			.onWhenAddingFileFailed = (item: any, filter: any, options: any) => {
				...
				if (item.size >= options.maxFileSize && item.type == this.fileTypes[index]) {
					this.sizeError = true;
					let toast: object;
	...
	fileValidation(item: any, showToast: any) {
		...
		if (item.type != this.fileTypes[index]) {
			...
			toast = { msg: 'Invalid file format.Supported file type(.jpg, .jpeg, .png, .pdf, .doc, .docx, .xlsx)', status: "error" };
			if (showToast) {
				this.snackbar.showSnackBar(toast);
			}
	...
	setAddedFilesUrl(event) {	
		this.uploader.setOptions({ url: App.base_url + 'uploadProductCertificate?products_id=' + event.id +'&org_id='+ this.Organization.id+'' })
	}
	fileSelected(event): void {
	}
	getAddedFiles(product_id) {		
		this.organizationsService
			.getCertAttachDetails({ product_id: product_id, org_id: this.Organization.id })
			.then(response => {
				if (response.result.success) {
					...
					this.certificationAttachments = response.result.data.product_attachments;
	...
	deleteUploads(file, i, productId) {
		this.organizationsService
			.deleteProductsFiles({ products_id: productId, attachments_id: file.att_id })
			.then(response => {
				if (response.result.success) {
					let toast: object;
