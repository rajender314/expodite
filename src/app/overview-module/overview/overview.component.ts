...
export class OverviewComponent implements OnInit {
  ...
  displayedColumns = ['order_no', 'client_name', 'po_nbr', 'date_added', 'total_amount', 'order_status'];
  ...
  getListOverview(): void {
    ...
    const today = new DatePipe('en-US').transform(this.today, "dd-MM-yyyy h:mm a"); 	

    this.OrdersService
      .getOverviewList({ date: today })
      .then(response => {
        this.fetchingData = false;
        if (response.result.success) {
          this.stock_available = response.result.data.stock_available;
          this.orders.veganRemaining = response.result.data.stock_available.length ? response.result.data.stock_available[0].sum: 0;
         ...
         // console.log(response.result.data.stock_available[1])
          this.orders.prod2 = response.result.data.stock_available[1] != undefined ? response.result.data.stock_available[1].name : '';
          this.orders.spectrumRemaining = response.result.data.stock_available[1] != undefined ? response.result.data.stock_available[1].sum : 0;
          ...
          this.orders.due = response.result.data.dueAndOverdueList ? response.result.data.dueAndOverdueList.Due : '';
          this.orders.overdue = response.result.data.dueAndOverdueList ? response.result.data.dueAndOverdueList.Overdue : '';
          ...
          this.orders.total_orders_count = response.result.data.total_orders_count;
