...
export class DeliverOrderComponent implements OnInit {
  ...
  this.uploader
    .onAfterAddingFile = (item: any) => {
      // this.uploadedFile = [];
      this.uploader.setOptions({ url: App.base_url + 'addFiles?orders_id=' + this.data.id + '&type=' + this.imagUploadFlag })
      ...
      .onCompleteItem = (item: any, response: any, status: any, headers: any) => {
        ...
        if (obj.result.success) {
          ...
          if (!obj.result.data.error_format) {
            this.uploadedFile = obj.result.data.OrdersAtt;
          } 
        } else {
          let toastMsg: object;
  ...
  getDeliveryInfo():void{
    this.OrdersService
      .getDeliverDetails({ order_id: this.data.id})
      .then(response => {
        if (response.result.success) {
          this.deliveryDate = response.result.data.delivery_date;
          this.uploadedFile = response.result.data.attach_details;
          this.setForm(this.deliveryDate, response.result.data.attach_details );
        }
      })
  }
  deleteUploads(item, i) {
    this.OrdersService
      .deleteFiles({ id: item.id, att_id: item.att_id })
      .then(response => {
        if (response.result.success) {
          let toast: object;
  ...
  deliverOrder(): void {
    this.showForm = false;
    this.loading = true;
    let diveryDate = moment(this.deliverOrderForm.value.delivery_date).format('YYYY-MM-DD');
    this.OrdersService
      .acceptOrder({ id: this.data.id, orders_types_id: 4, delivery_date: diveryDate })
      .then(response => {
        this.success = true;
        if (response.result.success) {
