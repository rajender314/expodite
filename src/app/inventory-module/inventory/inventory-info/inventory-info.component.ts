...
selectManufacringDate(event) {
  ...
  getBatchClientDetail(data?: any): void {
		this.InventoryService
			.getBatchClientDetails({ batch_id: this.invId })
			.then(response => {
				if (response.result.success) {
  ...
  getReportsData(data?: any): void {
		this.InventoryService
			.getBatchCoa({ batch_id: this.invId })
			.then(response => {
				if (response.result.success) {
  ...
  update(form: any) {
	...
	// console.log(form)
	// return
	if ((form.value.batch_nbr.trim() == '' || form.value.batch_nbr.trim() == null) || 
	(form.value.remain_quan == null || form.value.remain_quan == 0) ||
  ...
  getOrderDetails(data?: any): void {
		this.InventoryService
			.OrdersListBatch({ batches_id: this.invId })
			.then(response => {
				if (response.result.success) {
	...
	setForm(data: any): void {
		...
		this.minDate1.expiry_date =  new Date(data.mfd_date);
		this.prefillExpData =  new Date(data.exp_date)
		if(data.product_types_id) {
			this.arr.push(data.product_types_id)
		}
		// this.prefillExpData =  new Date(data.exp_date)
		const index = _.findIndex(this.filterProdList, { id: data.product_types_id });
		// console.log(index)
		if(index == -1) {
			this.filterProdList.push({id: data.product_types_id, name: data.product_name})
		}
		// console.log(this.filterProdList)
