...
export class InventoryComponent implements OnInit {
  ...
  getReportsData(data?: any): void {
    this.InventoryService.getBatchCoa({
  ...
  saveReportsTable(data?: any): void {
    ...
    this.reports.batchesCoaDt.forEach(function (res) {
      res.limits_label_val = res.limitsLabelVal
        ? res.limitsLabelVal
        : res.limits_label_val;
      res.result_label_val = res.resultLabelVal
        ? res.resultLabelVal
        : res.result_label_val;
      res.name = res.nameLabelVal ? res.nameLabelVal : res.name;
      params.batchesCoaDt.push(res);
    });

    let toast: object;
    this.InventoryService.saveReportData(params).then((response) => {
      if (response.result.success) {
  ...
  getBatchClientDetail(data?: any): void {
    this.InventoryService.getBatchClientDetails({
      batch_id: this.selectedBatch["id"],
    })
      .then((response) => {
        if (response.result.success) {
  ...
  getPagams(params?, flag?) {
    ...
    if (flag == "pagination") {
      data = {
        batchStatusArr: this.orders.slectedStatus.value,
        search: params.search,
        perPage: params.perPage,
        page: params.page,
        InvproductsArr: this.orders.slectedCatogory.value,
  ...
  clearInventory(): void {
    ...
    this.param.page = 1;
    this.toDate = "";
    this.orders.slectedCatogory.setValue([]);
  ...
  addParticalSize(data) {
    ...
    let index = _.findIndex(this.reports.batchesCoaDt, (obj: any) => {
    ...
    if (index > -1) {
      this.allCoaDetails = this.reports.batchesCoaDt.splice(
        index + 1,
        0,
        addPartical
      );
    ...
    this.timeout = setTimeout(() => {
      this.leadService.getGridList(params).then((response) => {
        if (response.result.success) {
          ...
          this.formBuilderBatchList = response.result.data.list;
  ...
  getOrderDetails(data?: any): void {
    this.InventoryService.OrdersListBatch({
      batches_id: this.selectedBatch["id"],
    }).then((response) => {
      if (response.result.success) {
    ...
    this.leadService
      ...
      .then((response) => {
        if (response.result.success) {
          ...
          this.formBuilderBatchList = response.result.data.list;
          this.totalCount = response.result.data.count;
          this.listCount = response.result.data.pageCount || 0;
        }
      })
      .catch((error) => console.log(error));
  }
  public orders = {
    ...
    slectedCatogory: new FormControl([]),
  }
  ...
  orderFilterData() {
    let param = {};
    this.OrdersService.getOrderFilterData({flag: "inventory"}).then((response) => {
      if (response.result.success) {
        this.statusList  = response.result.data.status;
        this.categoryList = response.result.data.categories;
      }
    });
  }
